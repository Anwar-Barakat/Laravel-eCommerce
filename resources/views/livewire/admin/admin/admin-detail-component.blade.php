<div>
                        ...
                        <div class="row row-cards">
                            <div class="mb-3">
                                <label class="form-label">
                                    {{ __('setting.roles') }}
                                    <a href="{{ route('admin.roles.index') }}">({{ __('msgs.add', ['name' => __('order.new')]) }})</a>
                                </label>
                                <div class="form-selectgroup">
                                    @foreach ($roles as $role)
                                        <label class="form-selectgroup-item" wire:key='role-{{ $role->id }}'>
                                            <input type="checkbox" value="{{ $role->id }}" class="form-selectgroup-input" wire:model="selectedRoles" />
                                            <span class="form-selectgroup-label">{{ ucwords(str_replace('_', ' ', $role->name)) }}</span>
                                        </label>
                                    @endforeach
                                </div>
    ...
    <div class="card mt-4">
        <div class="row g-0">
            <div class="col d-flex flex-column">
                <form wire:submit.prevent='submitPermission'>
                    <div class="card-body">
                        <h3 class="mb-4 text-blue">{{ __('msgs.main_info') }}</h3>
                        <div class="row row-cards">
                            <div class="mb-3">
